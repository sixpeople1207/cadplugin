...
namespace PipeInfo
{
               ...
               using (Transaction acTrans = db.TransactionManager.StartTransaction())
               {
                  ...
                  try
                  {
                     if (pRes.Status == PromptStatus.OK)
                     {
                        ...
                        List<Point3d> points = point3Ds.OrderByDescending(p => p.Y).ToList();

                        // Tee객체를 필터링한다. //끝객체 //Reducer
                        string[] filter = { "Tee", "Reducer", "Reducing" };

                        // 선택한 객체에서 필터를 걸러낸다.
                        List<Point3d> oldPoints = ddworks_Database.FilterWeldGroup_By_ComponentType(points, filter);

                        // 웰딩 포인트에 연결된 파이프를 찾아 Vector방향을 알아낸다.
                        // Spool 정보도 같이 불러온다. (맞대기 용접은 좌표를 더해서 반환)
                        (List<string> spoolInfo_li, List<Vector3d> vec_li, List<Point3d> newPoints) = ddworks_Database.Get_Pipe_Vector_By_WeldGroupPoints(oldPoints);
   ...
   * 기능 설명 : DDWorks_Database 에 관련된 기능.*/
   public class DDWorks_Database
   {
      ...
      */
      public (List<string>, List<Vector3d>, List<Point3d>) Get_Pipe_Vector_By_WeldGroupPoints(List<Point3d> weldGroup)
      {
