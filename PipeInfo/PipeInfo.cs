               ...
               using (Transaction acTrans = db.TransactionManager.StartTransaction())
               {
                  ...
                  try
                  {
                     if (pRes.Status == PromptStatus.OK)
                     {
                        ...
                        (List<string> spoolInfo_li, List<Vector3d> vec_li) = ddworks_Database.Get_Pipe_Vector_By_WeldGroupPoints(newPoints);

                        foreach (var v in vec_li)
                        {
                        ...
                        foreach(var spool in spoolInfo_li)
                        {
                           ed.WriteMessage(spool.ToString());
                        }
                           ...
                           if (newPoints.Count > 1)
                           {
                              ...
                              for (int k = 0; k < newPoints.Count; k++)
                              {
                                 ...
                                 textAlign[2] = (int)TextHorizontalMode.TextRight;
                                    ...
                                    text.Position = new Point3d(newPoints[k].X, newPoints[k].Y, newPoints[k].Z);
                                    ed.WriteMessage(text.HorizontalMode.ToString());
                                    ed.WriteMessage(text.AlignmentPoint.ToString());
                                    text.Height = 12.0;
                                 int nCnt = 0;
   ...
   * 기능 설명 : DDWorks_Database 에 관련된 기능.*/
   public class DDWorks_Database
   {
      ...
      */
       ...
       * 비고 : 1. PIPE의 맞대기 용접(WELD Point는 하나인데 Spool정보는 2개인 객체들은 weldGroup에서 매칭하는 알고리즘을 DrawText에서 구현)
       *        2. Vector 예시 (0,0,1)(0,0,-1)이 쌍으로 배치.
      */
      public (List<string>, List<Vector3d>) Get_Pipe_Vector_By_WeldGroupPoints(List<Point3d> weldGroup)
      {
